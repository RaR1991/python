...
class IPData:
    ...
    def lookup(self, ip=None, select_field=None, fields=[]):
        ...
        response = requests.get(
            f"{self.base_url}{query}", headers=self.headers, params=query_params
        )
        status_code = response.status_code
        if select_field and status_code == 200:
            try:
                response = {select_field: response.json(),
                            "status": status_code}
                return response
            except Exception:
    ...
    def bulk_lookup(self, ips=[], fields=[]):
        ...
        if len(ips) < 2:
            print(
                "Bulk Lookup requires more than 1 IP Address in the payload.\nUsing lookup function instead")
            try:
                self.lookup(ip=ips[0], fields=fields)
            ...
            response = requests.post(
                f"{self.bulk_url}", headers=self.headers, params=query_params, json=ips
            )
            status_code = response.status_code
